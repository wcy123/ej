...
-spec read_file(File :: string()) -> #state{}.
read_file(File) ->
...
modules_dir() ->
    DefaultDir = filename:join(ej_utils:getenv("HOME"), ".ejabberd-modules"),
    ej_utils:getenv("CONTRIB_MODULES_PATH", DefaultDir).
...
transform_terms_with_mod(Mod,Terms) ->
    case  erlang:function_exported(Mod,transform_options,1) of
        true -> catch Mod:transform_options(Terms);
        false -> {'EXIT', not_exported}
    end.
transform_terms([Mod|Mods], Terms) ->
    case transform_terms_with_mod(Mod,Terms)  of
        {'EXIT', _} = Err ->
...
transform_module_options(Module, Opts) ->
    Opts1 = try gen_iq_handler:transform_module_options(Opts)
            catch _:_ -> Opts                         %% bad style, work around
            end,
    try
